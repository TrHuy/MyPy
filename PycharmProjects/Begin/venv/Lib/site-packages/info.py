"""
A package for neatly displaying hierarchical information.
...
class Info:
    ...
    def _strings(self) -> List[str]:
        ...
        for field, value in self.children.items():
            if isinstance(value, Info):
                ...
                strings.append('├─ ' + info_strings[0])  # the header
                strings.extend('│  ' + info_string for info_string in info_strings[1:])
            else:
                strings.append(f'├─ {field}: {value}')

        return strings

    def _cleanup(self, strings) -> List[str]:
        # this loop goes over the field strings in reverse, cleaning up the tail of the structure indicators
        for index, field_string in reversed(list(enumerate(strings))):
            if field_string[0] == '├':  # this is the last branch on this level, replace it with endcap and break
                strings[index] = field_string.replace('├', '└')
                break
            else:  # not yet at last branch, continue cleanup
                strings[index] = field_string.replace('│', ' ', 1)
    ...
    def json(self):
        # TODO: recursive json output from Info
    ...
    def add_infos(self, *infos):
        """
        Add many Infos to this Info as sub-Infos.
    ...
    def extend(self, others):
        self.add_infos(*others)

    def __setitem__(self, key, value):
        self.add_field(key, value)

    def update(self, info):
        self.children.update(dict(info))
